...
class Handler(BaseHTTPRequestHandler):
    ...
    def _dashboard(self):
        ...
        # stacked horizontal bar: done | pending | none | skipped | error (a width-per-
        # segment track). denominator is total files; guards an empty table.
        bar_total = c_total_files or 1
        bar_segments = "".join(
            _seg(k, status_counts.get(k, 0), bar_total) for k in _STATUS_ORDER)
        stacked_bar = (
            f"<div class='bar'>{bar_segments}</div>"
            f"<p class='small' style='margin:6px 0 0'>{bar_total} file(s) total · "
            f"width ∝ count · hover a segment for its number</p>"
    ...
    # ===== petition legacy redirect (old /petition/<id> links → /txn/<txn_id>) =====
    ...
    def _review(self, rest: str):
        ...
        with connect() as conn, conn.cursor() as cur:
            ...
            cur.execute("""SELECT page_no, stage, verdict, comment FROM verdicts
                           WHERE sha256=%s AND declared_category=%s AND page_no IS NOT NULL
                           ORDER BY page_no""", (sha, declared))
            doctype_verdicts: dict[int, tuple[str, str]] = {}
            ocr_verdicts: dict[int, tuple[str, str]] = {}
            for pno, stage, verdict, comment in cur.fetchall():
                (doctype_verdicts if stage == 'doctype' else ocr_verdicts)[pno] = (verdict, comment)
        ...
        with connect() as conn, conn.cursor() as cur:
            ...
            cur.execute(f"""INSERT INTO verdicts (sha256, declared_category, page_no, stage, verdict,
                           ...
                           DO UPDATE SET verdict=EXCLUDED.verdict, comment=EXCLUDED.comment,
                                        created_at=now()""",
                        (sha, declared, page_n, stage, verdict, comment, "web"))
            conn.commit()
...
def _doc_types_block(ejson) -> str:
    """The per-page "Classify Doctype" block: pull the page's doc_types out of the
    extract JSON and render each is_* flag as a yes/no chip. The attchment
    extractor nests its flags under a 'doc_types' dict
    ...
    (a non-classifying extract or a failed run).
    """
...
def _page_section(sha: str, declared: str, pno: int, ocr_row: tuple,
                  doctype_verdict: tuple | None, ocr_verdict: tuple | None) -> str:
    ...
    The right-hand body is, in order:
      ...
      2. Classify Doctype block — the page's doc_types (is_*) pulled out of the
         JSON and shown as yes/no chips, so the per-page classification is legible
         at a glance instead of buried in the JSON dump.
      3. DocType verdict — is the doc_types classification right? True / False
         (POSTs stage='doctype', verdict 'correct'(=True)/'wrong'(=False)).
      4. OCR / ADE verdict — is the extracted data correct? Correct/Acceptable/Wrong
    ...
    doc_types_html = _doc_types_block(ejson) if ostatus == "done" else ""

    # DocType verdict (True/False). 'correct' = "the doc_types is right" (True);
    # 'wrong' = "it's not" (False). Reuses the correct/wrong button hues.
    dt_v = doctype_verdict
    dt_label = _verdict_pill(dt_v[0]) if dt_v else "<span class='small'>not yet</span>"
    dt_true = 'sel' if (dt_v and dt_v[0] == 'correct') else ''
    dt_false = 'sel' if (dt_v and dt_v[0] == 'wrong') else ''
    ...
    header = (f"Page {pno} · Extract <span class='{_status_dot(ostatus)}'>{esc(ostatus)}</span> "
              ...
              f" · doctype: {dt_label} · ocr: {oc_label}")
    ...
    <div class="doc-section review-card">
      ...
      <div class="page-row" style="border:none;padding:0;margin:0;align-items:stretch">
        ...
        <div class="page-body">
          ...
          {result_box}
          {doc_types_html}
          <div class="ocr-label">DocType verdict — is the doc_types correct?</div>
          <form method="post" action="/verdict" class="verdict-form doctype-verdict-form">
            ...
            <input type="hidden" name="stage" value="doctype">
            <div class="verdict-btns">
              <button type="button" class="verdict-btn verdict-correct {dt_true}" data-verdict="correct">✓ True</button>
              <button type="button" class="verdict-btn verdict-wrong {dt_false}" data-verdict="wrong">✗ False</button>
            </div>
            <input type="hidden" name="verdict" class="page_verdict" value="{esc(dt_val)}">
            <input type="submit" value="Save docType verdict" class="verdict-submit">
          </form>
          ...
          <form method="post" action="/verdict" class="verdict-form">
            ...
            <input type="submit" value="Save OCR verdict" class="verdict-submit">
          </form>
        </div>
      </div>
    </div>"""
