...
if __name__ == "__main__":
    ...
    for pred_path, gt_path in zip(pred_paths, gt_paths):
        ...
        for pred_meta, gt_meta in zip(pred_metas, gt_metas):
            ...
            ids.append(pred_meta["image"])
            preds.append(pred_score)
            #gts.append(gt_meta["gt_score"])
        
        output_lines = []
        for img_id, score in zip(ids, preds):
            output_lines.append(f"{img_id} {score}") # Format as "id score" per line

        with open(save_path, "w") as fw:
            fw.write("\n".join(output_lines)) # Join all formatted lines with newlines
