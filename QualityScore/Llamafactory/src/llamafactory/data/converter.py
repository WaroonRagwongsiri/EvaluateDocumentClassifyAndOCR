...
class SharegptDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        # 保留level_probs字段
        if "level_probs" in example:
            output["level_probs"] = example["level_probs"]
            #logger.info_rank0(f"Found level_probs in example: {example['level_probs']}")
        else:
            logger.warning_rank0("level_probs not found in example")
