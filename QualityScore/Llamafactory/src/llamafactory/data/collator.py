...
class SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        # 检查是否有level_probs
        has_level_probs = all("level_probs" in feature for feature in features)
        if has_level_probs:
            #logger.info_rank0("Found level_probs in features")
            # 收集level_probs
            level_probs = [feature.pop("level_probs") for feature in features]
            # 转换为tensor
            if isinstance(level_probs[0], torch.Tensor):
                level_probs = torch.stack(level_probs)
            else:
                level_probs = torch.tensor(level_probs)
            #logger.info_rank0(f"level_probs shape: {level_probs.shape}")
        else:
            logger.warning_rank0("level_probs not found in features")
            level_probs = None
        ...
        # 添加level_probs到features
        if level_probs is not None:
            features["level_probs"] = level_probs.to(self.compute_dtype)
            #logger.info_rank0(f"Added level_probs to features with shape: {features['level_probs'].shape}")
